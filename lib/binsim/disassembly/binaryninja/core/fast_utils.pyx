...
cpdef compute_function_hash(function:bn.Function):
    """
    ...
    for block in function:
        for tokens, _ in block:
            ins_tokens = []
            for token in tokens:
                if token.type in [token.type.InstructionToken,
                                    token.type.OperandSeparatorToken,
                                    token.type.RegisterToken,
                                    token.type.BeginMemoryOperandToken,
                                    token.type.EndMemoryOperandToken]:
                    ins_tokens.append(token.text)
                elif token.type in {token.type.TextToken,token.type.OperandSeparatorToken}:
                    ...
                    ins_text = "".join([t.text for t in tokens])
                    function_name = function.name
                    binary_file = function.view.file.filename
                    raise ValueError(f"Unknown token type: {token.type.name} in \"{ins_text}\" "
                                    f"@<{binary_file}:{function_name}")
