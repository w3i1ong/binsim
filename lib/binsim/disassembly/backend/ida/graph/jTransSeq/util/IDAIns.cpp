...
PYBIND11_MODULE(IDAIns, m) {
    m.doc() = "Module used to parse IDA instructions";
    py::class_<IDAIns>(m, "IDAIns")
            .def(py::init<const map<string, int>&>())
            .def("setToken2idx", &IDAIns::setToken2idx)
            .def("getToken2idx", &IDAIns::getToken2idx)
            .def("parseIns", &IDAIns::parseIns)
            .def("getOperator", &IDAIns::getOperator)
            .def("getOperands", &IDAIns::getOperands)
            .def("parseFunction", &IDAIns::parseFunction,
                 py::arg("ins_list"), py::arg("max_length"), py::arg("bb_length"));
}
