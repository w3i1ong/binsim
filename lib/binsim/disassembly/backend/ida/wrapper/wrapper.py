...
import psutil
import tempfile
from binsim.disassembly.backend.ida.graph.utils import HeartbeatServer, HeartbeatClient
...
class IDAExeWrapper:
    ...
    def extract_functions(self,
        ...
        kwargs_file = tempfile.mktemp(".kwargs")
        with open(kwargs_file, "wb") as f:
            pickle.dump(kwargs, f)
        command.add_script_kwargs("--kwargs", kwargs_file)
    ...
    def wait_process(self, command, binary_path, monitor_server: HeartbeatServer):
        ...
        while True:
            ...
            try:
                ...
                logger.error(
                    f"Timeout while disassembling {hex(func_addr)}@{binary_path}. Retrying...({retry_time}/{max_retry_time})")
                ...
                if max_retry_time < retry_time:
                    logger.error(f"Retry limit exceeded while disassembling {binary_path}.")
                    return None
                continue
            except RuntimeError as r:
                logger.error(f"RuntimeError while dealing with {binary_path}. Error message: {r}")
                return None
            except ValueError as e:
                logger.error(f"ValueError while dealing with {binary_path}. Error message: {e}.")
                proc.wait()
                # for child in psutil.Process(proc.pid).children(recursive=True):
                #     child.kill()
                return None
...
def execute_ida_script(ida_path,
    ...
    wrapper.extract_functions(binary,
                              outfile=out_file,
                              db_file=db_file,
